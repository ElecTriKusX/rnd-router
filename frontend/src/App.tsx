...
import {
  ArrowLeft, ArrowRight, Check, ChevronRight, CircleHelp, Clock3, Copy, FileText, Files,
  LoaderCircle, Mail, PanelLeftClose, PanelLeftOpen, Pencil, Plus, Search, Send,
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      {screen === 'compose' && <ComposeScreen {...{ selected, selectedSubtask, candidate: activeCandidate, facts, selectedFacts, setSelectedFacts, instruction, setInstruction, runDraft, loading, goBack: () => setScreen('results') }} />}
...
function RequestScreen(props: any) {
  ...
  return <section className="stage-content request-layout">
    <aside className="request-list panel"><div className="panel-heading"><div><h2>Заявки CRM</h2><p>Входящие обращения</p></div><button className="icon-button icon-button--primary" title="Добавить новую заявку" onClick={createRequest}><Plus size={17} /></button></div>
      <div className="request-search-row"><label className="search"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Найти заявку" /></label><button className="icon-button icon-button--filter" type="button" title="Фильтры"><SlidersHorizontal size={15} /></button></div>
...
function ResultsScreen({ selected, selectedSubtask, setSelectedSubtask, selectedCandidate, setSelectedCandidate, openCompose, goBack }: any) {
  ...
  return <section className="stage-content split-layout"><aside className="subtask-nav panel"><div className="subtask-nav__heading"><h2>Подзадачи</h2><p>Выберите направление подбора</p></div>{matches.map((item, index) => <button key={item.subtask.id} className={index === selectedSubtask ? 'subtask-nav__item subtask-nav__item--active' : 'subtask-nav__item'} onClick={() => { setSelectedSubtask(index); setSelectedCandidate(item.candidates[0] ?? null) }}><code>{String(index + 1).padStart(2, '0')}</code><span><small>{item.candidates.length} кандидата</small>{item.subtask.topic}</span></button>)}<div className="subtask-nav__bottom"><Status stage={selected.stage} /><button className="button button--secondary" onClick={goBack}><ArrowLeft size={15} />Изменить декомпозицию задач</button></div></aside>
    <section className="panel results"><div className="editor-heading"><div><h1>{current?.subtask.topic ?? 'Результаты подбора'}</h1><p>Кандидаты, релевантные выбранной подзадаче.</p></div><div className="selection-summary"><strong>{selectedCandidate ? 1 : 0} из {current?.candidates.length ?? 0} выбрано</strong><small>состав экспертной группы</small></div></div>{current && <div className="keyword-strip"><strong>Ключевые слова подзадачи:</strong>{current.subtask.keywords.map((word) => <em key={word}>{word}</em>)}</div>}
...
function ComposeScreen({ selected, selectedSubtask, candidate, facts, selectedFacts, setSelectedFacts, instruction, setInstruction, runDraft, loading, goBack }: any) {
  ...
  const subtask = selected?.subtasks?.[selectedSubtask]
  return <section className="stage-content split-layout"><aside className="reason-panel panel"><div className="reason-heading"><h2>Обоснование рекомендации</h2><p>Сформировано ИИ на основе заявки и профиля кандидата</p></div>{candidate && <><div className="reason-context"><code>ПОДЗАДАЧА {String(selectedSubtask + 1).padStart(2, '0')}</code><strong>{subtask?.topic ?? 'Выбранная подзадача'}</strong><div><span>РЕЛЕВАНТНОСТЬ</span><b>{candidate.score.toFixed(2)}</b></div></div><div className="reason-factors">{candidate.reasons.map((reason: string, index: number) => <article key={reason}><b>{String(index + 1).padStart(2, '0')}</b><p>{reason}</p></article>)}</div></>}<button className="button button--secondary" onClick={goBack}><ArrowLeft size={15} />Вернуться к результатам</button></aside>
